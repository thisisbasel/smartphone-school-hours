...
# Preprocessing -------------------------------------------------------------------------------------------------------
# Drop str columns 
df_blended_drop_str <- df_blended %>%
...
df_blended_daily_sum <- df_blended_drop_str %>%
  group_by(ID, date_local_original) %>%
  summarise(across(everything(), sum))

# Fill NaN values w 0s
df_blended_daily_sum_fill_nan <- df_blended_daily_sum %>%
  mutate_all(~ ifelse(is.na(.), 0, .))

# Calculate mean across days for each participant 
df_blended_participant_mean <- df_blended_daily_sum_fill_nan %>%
  select(-date_local_original) %>%
  group_by(ID) %>%
  summarise(across(everything(), mean, na.rm = TRUE)) 

# Select package names w/ _mins_impute tag
df_blended_app_usage <- df_blended_participant_mean %>%
...
colnames(df_blended_app_usage) <- ifelse(
  colnames(df_blended_app_usage) %in% df_app_categories_select$package,
  df_app_categories_select$category_tagged[match(colnames(df_blended_app_usage), df_app_categories_select$package)],
  colnames(df_blended_app_usage)
)

# If NaN, tag with no-category-info
colnames(df_blended_app_usage)[is.na(colnames(df_blended_app_usage))] <- "noCategoryInfo"

# Transpose df for combining matching categories 
transposed_df <- t(df_blended_app_usage) %>%
  as.data.frame() %>%
...
column_names <- c("Social", "Communication", "Gaming", "Video Players", "Productivity",
                  "Lifestyle", "Music & Audio", "Comics", "Educational", "Photography")
...
mean_values <- data_boxplot %>%
  group_by(column) %>%
  summarise(mean_value = mean(value))

# Create the horizontal box plot with mean annotations
ggplot_bloxplot_blended <- ggplot(data_boxplot, aes(y = column, x = value)) +
  geom_boxplot(
    horizontal = TRUE,
    fill = "#4C72B0",
    color = "#2F4F4F",
...
ggplot_bloxplot_blended

# Export the boxplot
ggsave(
  filename = "/Users/baselhussein/Projects/ttf/school-hours-analysis/vis/ggplot_boxplot_blended.png", 
  plot = ggplot_bloxplot_blended, 
