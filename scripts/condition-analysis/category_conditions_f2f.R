...
colnames(df_f2f_app_usage) <- ifelse(
  colnames(df_f2f_app_usage) %in% df_app_categories_select$package,
  df_app_categories_select$category_tagged[match(colnames(df_f2f_app_usage), df_app_categories_select$package)],
  colnames(df_f2f_app_usage)
)

# If NaN, tag with no-category-info
colnames(df_f2f_app_usage)[is.na(colnames(df_f2f_app_usage))] <- "noCategoryInfo"

# Transpose df for combining matching categories 
transposed_df <- t(df_f2f_app_usage) %>%
  as.data.frame() %>%
...
column_names <- c("Social", "Communication", "Video Players", "Gaming", "Entertainment",
                  "Music & Audio", "Photography", "Productivity", "Educational", "Travel & Local")
...
ggplot_bloxplot_f2f <- ggplot(data_boxplot, aes(y = column, x = value)) +
  geom_boxplot(
    horizontal = TRUE,
    fill = "#4C72B0",
    color = "#2F4F4F",
...
# Export the boxplot
ggsave(
  filename = "/Users/baselhussein/Projects/ttf/school-hours-analysis/vis/ggplot_boxplot_f2f.png", 
  plot = ggplot_bloxplot_f2f, 
