...
df_nonSchool <- read_csv("/Users/baselhussein/Projects/ttf/school-hours-analysis/data/condition/df_non_school_participants.csv")
...
# Preprocessing -------------------------------------------------------------------------------------------------------
# Drop str columns 
df_nonSchool_drop_str <- df_nonSchool %>%
...
df_nonSchool_daily_sum <- df_nonSchool_drop_str %>%
  group_by(ID, date_local_original) %>%
  summarise(across(everything(), sum))

# Fill NaN values w 0s
df_nonSchool_daily_sum_fill_nan <- df_nonSchool_daily_sum %>%
  mutate_all(~ ifelse(is.na(.), 0, .))

# Calculate mean across days for each participant 
df_nonSchool_participant_mean <- df_nonSchool_daily_sum_fill_nan %>%
  select(-date_local_original) %>%
  group_by(ID) %>%
  summarise(across(everything(), mean, na.rm = TRUE)) 

# Select package names w/ _mins_impute tag
df_nonSchool_app_usage <- df_nonSchool_participant_mean %>%
...
colnames(df_nonSchool_app_usage) <- ifelse(
  colnames(df_nonSchool_app_usage) %in% df_app_categories_select$package,
  df_app_categories_select$category_tagged[match(colnames(df_nonSchool_app_usage), df_app_categories_select$package)],
  colnames(df_nonSchool_app_usage)
)

# If NaN, tag with no-category-info
colnames(df_nonSchool_app_usage)[is.na(colnames(df_nonSchool_app_usage))] <- "noCategoryInfo"

# Transpose df for combining matching categories 
transposed_df <- t(df_nonSchool_app_usage) %>%
  as.data.frame() %>%
...
names(transposed_df_categories)[names(transposed_df_categories) == 'BooksAndReference'] <- 'Books'
names(transposed_df_categories)[names(transposed_df_categories) == 'TravelAndLocal'] <- 'Travel & Local'
...
column_names <- c("Social", "Video Players", "Communication", "Gaming", "Entertainment",
                  "Music & Audio", "Photography", "Productivity", "Books", "Travel & Local")
...
ggplot_bloxplot_nonSchool <- ggplot(data_boxplot, aes(y = column, x = value)) +
  geom_boxplot(
    horizontal = TRUE,
    fill = "#4C72B0",
    color = "#2F4F4F",
...
ggplot_bloxplot_nonSchool

# Export the boxplot
ggsave(
  filename = "/Users/baselhussein/Projects/ttf/school-hours-analysis/vis/ggplot_boxplot_nonSchool.png", 
  plot = ggplot_bloxplot_nonSchool, 
