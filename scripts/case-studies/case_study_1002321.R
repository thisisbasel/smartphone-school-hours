...
# 1002321 ------------------------------------------------------------------------------------------------------
# Subset data 
df_non_school_1002321 <- subset(df_non_school, ID == 1002321)
df_online_1002321 <- subset(df_online, ID == 1002321)
df_f2f_1002321 <- subset(df_f2f, ID == 1002321)
df_blended_1002321 <- subset(df_blended, ID == 1002321)

# Top Apps ---------------------------------------------------------------------------------------------------- 
# f2f ----
# Select hourly f2f data 
df_school_year_1002321 <- read.csv("/Users/baselhussein/Projects/ttf/school-hours-analysis/data/processed/df_school_year_1002321.csv")
df_school_year_1002321_crit1 <- df_school_year_1002321 %>%
  filter(crit1 == 'crit1') 

df_school_year_1002321_crit1_f2f <- df_school_year_1002321_crit1 %>%
  filter(instruction_condition == 'face-to-face' & day_type == 'Weekday' & holiday == 'None') # is.nan(holiday) not working...? 

# Unique days 
total_unique_days_f2f <- df_school_year_1002321_crit1_f2f %>% 
  summarise(total_unique_days = n_distinct(date_local_original))

# Keep only imputed app-usage
df_app_usage_f2f <- df_school_year_1002321_crit1_f2f %>%
...
# Online ----
df_school_year_1002321_crit1_online <- df_school_year_1002321_crit1 %>%
  filter(instruction_condition == 'online' & day_type == 'Weekday' & holiday == 'None')

# Unique days 
total_unique_days_online <- df_school_year_1002321_crit1_online %>% 
  summarise(total_unique_days = n_distinct(date_local_original))

# Keep only imputed app-usage
df_app_usage_online <- df_school_year_1002321_crit1_online %>%
  select(contains("_mins_impute"))
