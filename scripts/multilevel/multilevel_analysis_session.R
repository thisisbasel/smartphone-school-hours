...
df_non_school_grouped <- df_non_school %>%
  ...
  summarize(session_length_daily = sum(use_duration_minutes_impute) / sum(session_count_impute),
...
combined_conditions_seasons_drop <- combined_conditions_seasons[complete.cases(combined_conditions_seasons$session_length_daily), ]
...
model <- lmer(session_length_daily ~ # Response variable 
...
# Review model output 
sink(file = "session_analysis.txt")
